...
internal class Game
{
    ...
    {
        // Check if string from MainWindow is Empty
        if (wnd.PromotedTo == "" || wnd.PromotedTo == null)
        {
            return;
        }

        switch (wnd.PromotedTo)
        {
            case "Queen":
                {
                    // add the new piece at the position
                    Pieces.Add(new Queen(currentplayer.SelectedPiece.Position, currentplayer.IsWhite));
                    // set the piece to killed to remove it
